...
const JoinMortgageLO = () => {
  ...
  const steps = [
    ...
    { id: 2, title: "Experience", description: "Background Info" },
    { id: 3, title: "Contact Info", description: "Name & Email" },
    { id: 4, title: "Phone Number", description: "Verify & Connect" },
    { id: 5, title: "What You Want", description: "Career Goals" },
    { id: 6, title: "Get Matches", description: "Receive Offers" }
  ];

  const loanTypeOptions = ["Purchase", "Refinance", "FHA", "VA", "Jumbo", "Conventional"];
  const wantsOptions = [
    "Higher Comp",
    "More Leads",
    "Better Technology",
  ...
  const validateStep = () => {
    switch (currentStep) {
      ...
      case 2:
        ...
        if (!formData.annualLoans) {
          toast.error("Please select your annual loan volume");
          return false;
        }
        if (formData.loanTypes.length === 0) {
          toast.error("Please select at least one loan type");
          return false;
        }
        break;
      case 3:
    ...
    try {
      const { data, error: signupError } = await supabase.functions.invoke('agent-directory-signup', {
        body: {
          ...
          annual_loans: formData.annualLoans,
  ...
  const toggleWant = (want: string) => {
    setFormData(prev => ({
      ...prev,
      wants: prev.wants.includes(want)
        ? prev.wants.filter(w => w !== want)
        : [...prev.wants, want]
    }));
  };
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-gray-700 font-semibold">Years in Mortgage</Label>
              ...
              <div className="space-y-2">
                <Label className="text-gray-700 font-semibold">Annual Loan Volume</Label>
                <div className="grid grid-cols-2 gap-3">
                  {["1-20", "21-50", "51-100", "100+"].map(volume => (
                    <button
                      key={volume}
                      onClick={() => setFormData({ ...formData, annualLoans: volume })}
                      className={`p-4 rounded-xl border-2 transition-all ${
                        formData.annualLoans === volume
                          ? "border-[#35a87e] bg-[#35a87e]/5 text-[#35a87e] font-semibold"
                          : "border-gray-200 hover:border-[#35a87e]/50"
                      }`}
                    >
              ...
              <div className="space-y-2">
                <Label className="text-gray-700 font-semibold">Loan Types (select all that apply)</Label>
                <div className="grid grid-cols-2 gap-3">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {wantsOptions.map(want => (
                  <button
                    key={want}
                    onClick={() => toggleWant(want)}
                    className={`p-4 rounded-xl border-2 transition-all text-left ${
                      formData.wants.includes(want)
                        ? "border-[#35a87e] bg-[#35a87e]/5 text-[#35a87e] font-semibold"
                        : "border-gray-200 hover:border-[#35a87e]/50"
                    }`}
                  >
          ...
          <div className="flex-1 flex items-center justify-center relative z-10">
            <div className="w-full max-w-sm">
              <div className="bg-white/95 backdrop-blur-md rounded-2xl p-8 shadow-2xl border border-white/20">
                <h3 className="text-2xl font-bold mb-8 text-gray-900">Loan Officer Signup</h3>
                <div className="space-y-6">
                  {steps.map(step => (
